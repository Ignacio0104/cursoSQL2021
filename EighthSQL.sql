...
#create view NOMBREVISTA as SENTENCIASSELECT from TABLA; --> Sintaxis
...
create view vista_empleado as select concat(apellido, ' ', e.nombre) as 'Nombre', sexo as 'Sexo', s.nombre as 'Seccion',
cantidadhijos as 'Cantidad de hijos' from empleados as e join secciones as s on codigo=seccion;

select * from vista_empleado;

select seccion, count(*) as cantidad from vista_empleado group by seccion;
create view vista_seccion as select seccion, count(*) as cantidad from vista_empleado group by seccion;

select * from vista_seccion;
drop view if exists vista_empleado;
 
#create view NOMBREVISTA (NOMBRESDEENCABEZADOS) as SETENCIASSELECT from TABLA; -->Sintaxis 2
create view vista_empleado_ingreso (FechaIngreso,Cantidad) as select extract(year from fechaingreso) as
fecingreso, count(*) as cantidad from empleados group by fecingreso;

select * from vista_empleado_ingreso;
drop view if exists vista_empleado_ingreso;
...
values('86876768', 'Juan Pablo', 10, 1); #Esto se agrega a lista original tambien
